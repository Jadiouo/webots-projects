...
|---|---|---|---|
| `light-follower/` | `light_follower.wbt` | `light_follower` | 四輪車 + 兩顆光感測器的趨光控制：以左右亮度差做非線性轉向，靠近光源時提高轉向增益並減速 |
...
| `arm-rl/` | `arm_rl.wbt` | `rl_controller` | 手臂強化學習環境：以 Supervisor 讀取末端與目標球（`TARGET_A/B/C`）世界座標，提供 observation / reward / action 介面，目標點 A→B→C 循環；目前 action 為隨機擾動，留給 RL 模型接入 |
